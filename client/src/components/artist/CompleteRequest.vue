    ...
    <div v-if="paginatedArtworks.length > 0" class="artwork-wrap">
      ...
      >
        ...
        <div class="button-section">
          <button @click="applyCompletedReview(artwork.artworkId)">
            완결 신청
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
      ...
      if (email) {
        try {
          const response = await fetch(
            `/api/artworks/approved-normalBreak?email=${email}`
          )
          if (response.ok) {
    ...
    async applyCompletedReview (artworkId) {
      try {
        const response = await fetch(
          `/api/artworks/${artworkId}/status-CompletedReview`,
          {
            method: 'PUT'
          }
        )
