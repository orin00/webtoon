    ...
    <div v-if="paginatedArtworks.length > 0" class="artwork-wrap">
      ...
      >
        ...
        <div class="button-section">
          <button @click="applyNormalReview(artwork.artworkId)">
            휴재복귀 신청
          </button>
        </div>
      </div>
...
export default {
  ...
  methods: {
      ...
      if (email) {
        try {
          const response = await fetch(
            `/api/artworks/approved-break?email=${email}`
          )
          if (response.ok) {
    ...
    async applyNormalReview (artworkId) {
      try {
        const response = await fetch(
          `/api/artworks/${artworkId}/status-NormalReview`,
          {
            method: 'PUT'
          }
        )
