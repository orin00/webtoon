    ...
    >
      ...
      >
        ...
        <div class="button-section">
          <button @click="approveCompleted(artwork.artworkId)">
            완결 승인
          </button>
          <button @click="denyCompleted(artwork.artworkId)">
            완결 거부
          </button>
        </div>
      </div>
...
export default {
  ...
  mounted () {
    this.fetchCompletedReviewArtworks()
  },
  methods: {
    async fetchCompletedReviewArtworks () {
      try {
        const response = await fetch('/api/artworks/CompletedReview')
        if (response.ok) {
    ...
    async approveCompleted (artworkId) {
      console.log(`Approving artwork with ID: ${artworkId}`) // 로그 추가
      try {
        const response = await fetch(`/api/artworks/${artworkId}/status-Completed`, {
          method: 'PUT'
        })
        if (response.ok) {
          const updatedArtwork = await response.json()
          console.log('Artwork updated:', updatedArtwork)
          this.fetchCompletedReviewArtworks() // 다시 작품 리스트를 가져옴
        } else {
    ...
    async denyCompleted (artworkId) {
      console.log(`Denying break for artwork with ID: ${artworkId}`) // 로그 추가
      try {
        const response = await fetch(`/api/artworks/${artworkId}/status-Completed-Denied`, {
          method: 'PUT'
        })
        if (response.ok) {
          const updatedArtwork = await response.json()
