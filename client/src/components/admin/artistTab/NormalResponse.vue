    ...
    <div v-if="paginatedArtworks.length > 0" class="artwork-wrap">
      ...
      >
        ...
        <div class="button-section">
          <button @click="approveNormal(artwork.artworkId)">복귀 승인</button>
          <button @click="denyNormal(artwork.artworkId)">복귀 거부</button>
        </div>
      </div>
...
export default {
  ...
  mounted () {
    this.fetchNormalReviewArtworks()
  },
  methods: {
    async fetchNormalReviewArtworks () {
      try {
        const response = await fetch('/api/artworks/NormalReview')
        if (response.ok) {
    ...
    async approveNormal (artworkId) {
      console.log(`Approving artwork with ID: ${artworkId}`) // 로그 추가
      try {
        const response = await fetch(
          `/api/artworks/${artworkId}/status-Normal`,
          {
            method: 'PUT'
          }
        )
        if (response.ok) {
          ...
          this.fetchNormalReviewArtworks() // 다시 작품 리스트를 가져옴
        } else {
    ...
    async denyNormal (artworkId) {
      console.log(`Denying break for artwork with ID: ${artworkId}`) // 로그 추가
      try {
        const response = await fetch(
          `/api/artworks/${artworkId}/status-Normal-Denied`,
          {
            method: 'PUT'
          }
        )
